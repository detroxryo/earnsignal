...
- Generic GitHub reward discovery is fail-closed until a dedicated platform adapter verifies funding, payout terms, and operator-region eligibility. For the two highest-reward rows, bounded comment-edge and issue-timeline enrichment samples at most two pages each; linked same-repository pull requests lower success estimates, while unavailable, incomplete, malformed, or unsampled competition evidence receives a conservative fallback. Platform-bot evidence can add a more specific region rejection; production is configured for `CN`.
